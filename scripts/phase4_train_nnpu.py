...
from ifg26.evaluation.ranking_metrics import compute_ndcg, recall_at_k
...
class MLP(nn.Module):
    ...
    def score_logit(self, x):
        """Return scalar score BEFORE sigmoid."""
        # Layers: (Linear, [BN], ReLU, Dropout) * N, then Linear, then Sigmoid
        # Sigmoid is the last layer (-1).
...
def load_features(cfg: dict) -> dict:
    ...
    esm2_path = feats_dir / "protein_embeddings_esm2_650m.parquet"
    if not esm2_path.exists():
        print(f"WARN: ESM2 cache not found at {esm2_path}. LP0 will use placeholders.")
        esm2_df = pd.DataFrame(columns=["uniprot_id", "embedding"])
    else:
        esm2_df = pd.read_parquet(esm2_path)
    
    # Map UniProt ID -> Embedding
    esm_map = dict(zip(esm2_df["uniprot_id"], esm2_df["embedding"]))
...
def make_protein_esm_embeddings(df: pd.DataFrame, esm_map: dict) -> torch.Tensor:
    # First embedding found determines dimensionality
    embed_dim = 1280 # Default for esm2_t33_650M
    if esm_map:
        for v in esm_map.values():
            embed_dim = len(v); break
            
    e3_ids = df["e3_uniprot_id"].fillna("").astype(str)
    t_ids  = df["target_uniprot_id"].fillna("").astype(str)
    
    def get_vec(uid):
        res = esm_map.get(uid)
        return np.array(res, dtype=np.float32) if res is not None else np.zeros(embed_dim, dtype=np.float32)

    e3_vecs = np.array([get_vec(u) for u in e3_ids])
    t_vecs  = np.array([get_vec(u) for u in t_ids])
    
    return torch.from_numpy(np.hstack([e3_vecs, t_vecs]))


def make_features(df: pd.DataFrame, model_type: str,
                  ecfp_mat: np.ndarray, n_proteins: int, 
                  esm_map: dict = None) -> torch.Tensor:
    ecfp = make_ecfp_tensor(df, ecfp_mat)
    if model_type == "L0":
        return ecfp
    elif model_type == "LP0":
        if esm_map is not None:
            prot = make_protein_esm_embeddings(df, esm_map)
        else:
            # Fallback to zero-vector placeholder if no ESM map is provided
            # (Ensures compatibility if generate_protein_embeddings.py wasn't run)
            prot = torch.zeros((len(df), 2560)) # 1280 * 2
...
def train_model(
    ...
    val_p_X: torch.Tensor, val_u_X: torch.Tensor, pi: float,
    ...
    loss_mode = train_cfg.get("loss_mode", "ranking") # Default to ranking per objective
    ...
    if loss_mode == "ranking":
        ...
        lg.info(f"    [{model_tag}] Training with Pairwise Ranking Loss...")
    else:
        lg.info(f"    [{model_tag}] Training with Pointwise nnPU Loss...")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for (p_X,) in p_loader:
            ...
            if loss_mode == "ranking":
                # Ensure equal batch sizes for pairwise
                bs = min(len(p_X), len(u_X))
                # DualModalityMLP.score_logit if available, else raw model forward
                p_score = model.score_logit(p_X[:bs]) if hasattr(model, 'score_logit') else model(p_X[:bs])
                u_score = model.score_logit(u_X[:bs]) if hasattr(model, 'score_logit') else model(u_X[:bs])
                loss = criterion(p_score, u_score, pi)
            else:
                pos_out = model(p_X)
                unl_out = model(u_X)
        ...
        # Val: For ranking, we use Recall@5% on val set as better early stopping signal
        model.eval()
        with torch.no_grad():
            p_val_sc = model(val_p_X)
            u_val_sc = model(val_u_X)
            y_true = np.concatenate([np.ones(len(p_val_sc)), np.zeros(len(u_val_sc))])
            y_scores = np.concatenate([p_val_sc.cpu().numpy(), u_val_sc.cpu().numpy()])
            val_sc = recall_at_k(y_true, y_scores, 5.0)
            
        val_scores.append(val_sc)

        if epoch % 10 == 0:
            lg.info(f"    [{model_tag} π={pi}] epoch {epoch:3d} | "
                    f"loss={avg_loss:.4f} | R@5%={val_sc:.4f}")
...
def main():
    ...
    for model_type in cfg["nnpu"]["models"]:
        ...
        X_p_train = make_features(p_train, model_type, ecfp, n_prot, esm_map=feats["esm_map"])
        X_p_val   = make_features(p_val,   model_type, ecfp, n_prot, esm_map=feats["esm_map"])
        X_p_test  = make_features(p_test,  model_type, ecfp, n_prot, esm_map=feats["esm_map"])

        # U splits (unlabeled)
        X_u_all   = make_features(pool_U, model_type, ecfp, n_prot, esm_map=feats["esm_map"])
        
        u_train   = pool_U[pool_U["source_split"] == "train"]
        u_val     = pool_U[pool_U["source_split"] == "val"]
        
        X_u_train = make_features(u_train, model_type, ecfp, n_prot, esm_map=feats["esm_map"]) \
                    if len(u_train) > 0 else X_u_all[:min(len(X_u_all), len(X_p_train))]
        X_u_val   = make_features(u_val,   model_type, ecfp, n_prot, esm_map=feats["esm_map"]) \
                    if len(u_val) > 0 else X_u_all[:min(len(X_u_all), 1000)]
        ...
        for pi in pi_grid:
            ...
            if model_type == "LP0":
                # Determine protein dimension from data
                protein_feat_dim = (in_dim - 2048) // 2
                model = DualModalityMLP(
                    ligand_dim=2048,
                    protein_dim=protein_feat_dim,
                    ligand_hidden=mlp_cfg.get("ligand_hidden", [512, 256]),
                    protein_hidden=mlp_cfg.get("protein_hidden", [512, 256]),
                    fusion_hidden=mlp_cfg.get("fusion_hidden", [256, 128]),
            ...
            model, train_losses, val_scores = train_model(
                ...
                X_p_val, X_u_val, pi, cfg, lg, model_tag,
            )
